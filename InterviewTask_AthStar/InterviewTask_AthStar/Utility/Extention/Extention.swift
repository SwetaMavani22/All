...
extension NSMutableAttributedString {
    ...
    func setUnderlineForText(textForAttribute: String) {
        let range: NSRange = self.mutableString.range(of: textForAttribute, options: .caseInsensitive)
        self.addAttributes([.underlineStyle: NSUnderlineStyle.single.rawValue, .font: UIFont.systemFont(ofSize: 16)], range: range)
    }
}
...
class MenuBubbleView: UIView {
    ...
    override func layoutSubviews() {
        ...
        let bezierPath = UIBezierPath(roundedRect: self.bounds,
                                      ...
                                      cornerRadii: CGSize(width: 70.0, height: 70.0))
